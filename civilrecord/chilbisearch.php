...
<div style="width:800px; height:330px; background-color:#999900; margin:0px auto 0px auto"><center><h1 style="table-layout:auto; color:#663333;">BORN SEARCH</h1></center><?php include('connection.php'); ?>

<body>
   <div style=" max-height:200px; max-width:900px; overflow:scroll">   
	    <center> <form method="post" action="chilbiupdate.php">
 <table><tr><td> <input name="search" type="text" /></td><td> <input type="submit" value="Search" /></td></tr></table>
              ...
              <center>  <table cellpadding="2" cellspacing="2" border="2" bordercolor="#000000" class="table table-striped table-bordered" >
                    ...
                    <thead >
                         <tr> <th>id</th><th>date</th><th>First Name</th><th>Laste Name</th><th>Date Of Birth</th><th>Gender</th><th>Father Names</th><th>Mother Names</th><th>cellule</th><th>Vallage</th><th>Status</th><th>UPDATE</th></tr>
                    </thead>
                    <tbody>
                        <?php
                        $search=$_REQUEST['search'];
$query="select * from chlidrenbirth where ch_id like '%$search%' or date like '%$search%' or chfname like '%$search%' or chlname like '%$search%' or dateOfbirth like '%$search%' or gender like '%$search%' or fathnames like '%$search%' or matnames like '%$search%' or cell like '%$search%' or vallege like '%$search%' or alive like '%$search%'";
$query1=mysql_query($query);
              ...
              echo '<td>' . $row['ch_id'] . '</td>';
				 echo '<td>' . $row['date'] . '</td>';
				 echo '<td>' . $row['chfname'] . '</td>';
				 echo '<td>' . $row['chlname'] . '</td>';
				 echo '<td>' . $row['dateOfbirth'] . '</td>';
				echo '<td>' . $row['gender'] . '</td>';
				echo '<td>' . $row['fathnames'] . '</td>';
				echo '<td>' . $row['matnames'] . '</td>';
				echo '<td>' . $row['cell'] . '</td>';
				echo '<td>' . $row['vallege'] . '</td>';
				echo '<td>' . $row['alive'] . '</td>';
				
	 echo"<td ><a href ='chilbiup.php?ch_id=$id'>";?><button type="button" name="Submit" onClick="return confirm('You Are Going to Update Record!!')">Update</button><?php print"</a></td>";
